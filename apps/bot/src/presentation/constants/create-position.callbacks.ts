...
export const CP_CALLBACKS = {
  strategy: (name: "spot" | "curve" | "bid-ask") => `${CP_PREFIX}:strategy:${name}`,
  depositMethod: (method: "sol_auto_convert" | "single_sided") => `${CP_PREFIX}:deposit:${method}`,
...
export const CP_PATTERNS = {
  strategy: new RegExp(`^${CP_PREFIX}:strategy:(spot|curve|bid-ask)$`),
