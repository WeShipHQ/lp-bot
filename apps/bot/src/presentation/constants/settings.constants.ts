...
export type RebalanceSchedule = "5m" | "15m" | "1h" | "3h" | "disabled" | "custom";
...
export const ST_PATTERNS = {
  ...
  scheduleSet: new RegExp(`^${ST_PREFIX}:schedule:set:(5m|15m|1h|3h|disabled|custom)$`),
  
  // Rebalancing settings
  toggleRebalance: new RegExp(`^${ST_PREFIX}:rebalance:toggle$`),
  rebalanceThreshold: new RegExp(`^${ST_PREFIX}:threshold:set:(10|15|20|25|30|custom)$`),
