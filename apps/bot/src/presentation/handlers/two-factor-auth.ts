...
export async function handleTwoFactorSetup(
  ...
  try {
    ...
    await ctx.reply("Error setting up 2FA");
  }
}
...
async function generateAndSend2FASetup(
  ctx: BotContext,
  userInfo: UserInfo,
  telegramUserId: string
) {
  try {
      ...
      { source: Buffer.from(setup.qrCodeUrl.split(",")[1], "base64") },
      {
        ...
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "✅ I'm Ready to Verify",
                callback_data: "verifyTwoFactorReady",
              },
            ],
          ],
        },
    ...
    await ctx.reply("Failed to setup 2FA");
  }
}
...
export async function handleTwoFactorVerification(
  ...
  try {
    ...
    await ctx.reply("Error verifying 2FA code");
  }
}
...
async function enable2FAForUser(
  ...
  try {
    ...
    await ctx.reply("Failed to enable 2FA");
  }
}
...
export async function handleTwoFactorDisable(ctx: BotContext) {
  try {
    ...
    await ctx.reply("Error processing 2FA disable request");
  }
}
...
export async function handleTwoFactorStatus(ctx: BotContext) {
  try {
    ...
    await ctx.reply(
      // MessageService.getTwoFactorStatusMessage(
    ...
    await ctx.reply("Error getting 2FA status");
  }
}
