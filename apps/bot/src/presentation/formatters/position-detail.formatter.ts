...
export class PositionDetailFormatter {
  static format(params: FormatParams): PositionDetailView {
    ...
    const currentValueUsd = onchain?.currentValueUsd ?? position.getCurrentValue().toNumber();
    ...
    const netProfitUsd = currentValueUsd + claimedFeesUsd + unclaimedFeesUsd - initialValueUsd;
    const netProfitPct = initialValueUsd > 0 ? (netProfitUsd / initialValueUsd) * 100 : 0;
    ...
    const balanceLines: BalanceLine[] = [
      {
        ...
        usd: tokenAPrice > 0 ? formatCurrency(tokenAUsd, { maxDecimals: 2 }) : undefined,
      },
      {
        label: tokenB.symbol,
        amount: tokenBAmount.toFormattedString(),
        usd: tokenBPrice > 0 ? formatCurrency(tokenBUsd, { maxDecimals: 2 }) : undefined,
      },
    ];
    ...
    const lowerBinId = typeof metadata.lowerBinId === "number" ? metadata.lowerBinId : undefined;
    const upperBinId = typeof metadata.upperBinId === "number" ? metadata.upperBinId : undefined;
    const activeId = typeof metadata.activeId === "number" ? metadata.activeId : undefined;

    const lines: string[] = [];

    lines.push(`*${pairLabel}* (${position.dex.toUpperCase()})`);
    lines.push("");
    lines.push(`${pnlEmoji} *Net PnL:* ${formatCurrency(netProfitUsd, { maxDecimals: 2 })} (${formatPercentage(netProfitPct, { decimals: 2, alwaysShowSign: true })})`);
    lines.push(divider());
    ...
    lines.push(`• Total Value: ${formatCurrency(currentValueUsd, { maxDecimals: 2 })}`);
    lines.push("");

    lines.push(`*Fees*`);
    lines.push(`• Claimed: ${formatCurrency(claimedFeesUsd, { maxDecimals: 2 })}`);
    lines.push(`• Unclaimed: ${formatCurrency(unclaimedFeesUsd, { maxDecimals: 2 })}`);
    lines.push("");

    lines.push(`*Status*`);
    lines.push(`• ${statusLabel}`);
    if (typeof lowerBinId === "number" && typeof upperBinId === "number") {
      const activeSuffix = typeof activeId === "number" ? ` (active: ${activeId})` : "";
      lines.push(`• Bin Range: ${lowerBinId} – ${upperBinId}${activeSuffix}`);
    }
    if (pool?.currentPrice != null) {
      lines.push(`• Pool Price: ${formatNumber(pool.currentPrice, { maxDecimals: 6 })} ${pool.tokenA.symbol}/${pool.tokenB.symbol}`);
    }
    lines.push("");
