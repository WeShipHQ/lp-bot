...
} from "../handlers/two-factor-auth";
import { tempStoreService } from "@/services/temp-store.service";
import {
  getTwoFactorKeyboard,
  getTwoFactorSetupKeyboard,
} from "../keyboards/two-factor-menu";
...
async function handleVerifyReady(ctx: BotContext) {
  try {
    await ctx.answerCbQuery(
      "✅ Ready to verify! Please enter your 6-digit code."
    );
...
export function twoFactorAuthCommand(
  ...
  // Action handlers
  bot.action("setupTwoFactor", (ctx) =>
    handleActionWithError(ctx, "🔐 Setting up 2FA...", () =>
      handleTwoFactorSetup(ctx, server)
    )
  );
  bot.action("twoFactorStatus", (ctx) =>
    handleActionWithError(ctx, "📊 Checking 2FA status...", () =>
      handleTwoFactorStatus(ctx)
    )
  );
  bot.action("disableTwoFactor", (ctx) =>
    handleActionWithError(ctx, "❌ Disabling 2FA...", () =>
      handleTwoFactorDisable(ctx)
    )
  );
  bot.action("verifyTwoFactorReady", (ctx) => handleVerifyReady(ctx));

  // Navigation actions
  bot.action("back_to_2fa_menu", (ctx) =>
    handleActionWithError(ctx, "🔙 Back to 2FA menu", async () => {
      await ctx.editMessageText(
        "🔐 **Two-Factor Authentication**\n\n" +
          "Choose an option below to manage your 2FA settings:\n\n" +
          "• **Setup 2FA** - Enable Two-Factor Authentication\n" +
