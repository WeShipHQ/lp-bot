...
import { ResilienceError, getUserMessage } from '@/utils/errors';

export class ErrorHandler {
  handle(
    error: unknown,
    context?: Record<string, any>
  ): { userMessage: string; logLevel: 'debug' | 'info' | 'warn' | 'error' } {
    ...
    if (error instanceof RateLimitError) {
      return {
        userMessage: '⚠️ Too many requests. Please slow down and try again later.',
        logLevel: 'warn',
      };
    }
    ...
    if (error instanceof ExternalApiError) {
      return {
        userMessage: '⚠️ External service is currently unavailable. Please try again later.',
        logLevel: 'warn',
      };
    }
    ...
    if (error instanceof BlockchainError) {
      return {
        userMessage: '⚠️ Network issue with Solana RPC. Please try again shortly.',
        logLevel: 'warn',
      };
    }
    ...
    if (error instanceof DatabaseError) {
      return {
        userMessage: '⚠️ Temporary database issue. Please try again later.',
        logLevel: 'error',
      };
    }
