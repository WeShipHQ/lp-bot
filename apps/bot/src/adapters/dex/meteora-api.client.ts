...
export class MeteoraApiClient {
  ...
  async getPool(poolAddress: string): Promise<MeteoraDlmmPoolResponse> {
    ...
    try {
      ...
      console.error(`[MeteoraApiClient] Failed to fetch pool ${poolAddress}`, error);
      throw error;
    }
  }
  ...
  async getTrendingPools(
    ...
    if (params.include_token_mints)
      params.include_token_mints.forEach((m) => url.searchParams.append("include_token_mints", m));
    ...
    try {
      return await api.getWithRetry<MeteoraDlmmPoolsPaginationResponse>(url.toString(), undefined, {
        maxRetries: this.retries,
        exponentialBackoff: true,
      });
    } catch (error) {
  ...
  async getAllPools(): Promise<MeteoraDlmmPoolResponse[]> {
    ...
    try {
      const res = await api.getWithRetry<{ pairs?: MeteoraDlmmPoolResponse[] } | MeteoraDlmmPoolResponse[]>(
        url,
        undefined,
        {
          maxRetries: this.retries,
      ...
      if (res && Array.isArray((res as any).pairs)) return (res as any).pairs as MeteoraDlmmPoolResponse[];
      return [];
    } catch (error) {
