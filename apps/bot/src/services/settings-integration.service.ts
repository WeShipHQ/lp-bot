...
export class SettingsIntegrationService {
   ...
   */
  async shouldAutoRebalancePosition(
    telegramId: string,
    position: any
  ): Promise<boolean> {
    const userSettings = await this.settingsService.getUserSettings(telegramId);
    return await this.settingsService.shouldAutoRebalance(
      position,
      userSettings
    );
  }
   ...
   */
  async shouldTakeProfitPosition(
    telegramId: string,
    position: any
  ): Promise<boolean> {
    const userSettings = await this.settingsService.getUserSettings(telegramId);
   ...
   */
  async updateUserRebalanceSchedule(
    telegramId: string,
    schedule: string
  ): Promise<void> {
   ...
   */
  async updateStopLossPercentage(
    telegramId: string,
   ...
   */
  async updateTakeProfitPercentage(
    telegramId: string,
    percentage: number | null
  ): Promise<void> {
    if (percentage) {
      ...
      logger.info(
        `Updated take profit to ${percentage}% for user ${telegramId}`
      );
    }
  }
   ...
   */
  async updateSlippagePercentage(
    telegramId: string,
    percentage: number
  ): Promise<void> {
    if (percentage) {
      const updater = container.get(UpdateUserSettingUseCase);
      await updater.setSlippagePercentage(telegramId, percentage);
  ...
  async updateDefaultBinRange(
    ...
    logger.info(
      `Updated default bin range to ${binRange} for user ${telegramId}`
    );
  }
  ...
  async updateAutoConvertToSol(
    ...
    logger.info(
      `Updated auto convert to SOL to ${enabled} for user ${telegramId}`
    );
  }
}
